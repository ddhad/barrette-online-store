...
def parse_table(folder_path):
    ...
    for raw_row in wb[sheet].iter_rows(min_row=2, values_only=True):
        ...
        product_type, en_name, short_description,\
            description, usage, ingredients, benefits, size = row[5:]

        product_folder = get_images_dir(str(barcode), images_directories)
        if product_folder:
            images_paths = [
                f'{product_folder}/{i}' for i in os.listdir(product_folder)
            ]
        ...
        CURSOR.execute(
            ...
            "(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? ,? ,? ,?)",
            (
                barcode, bg_name, en_name, material_code, series,
                series_description, product_type, short_description,
                description, usage, ingredients, benefits, size, repr(images)
            )
        )
